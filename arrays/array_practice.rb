...
puts "If we delete 5, 6 and 7 from the array, we're left with #{numbers.delete_if { |number| [5,6,7].include? number }}"
puts "If we add 5 at the beginning of the array, we're left with #{numbers.insert(0,5)}"
puts "If we add 6 at the end of the array, we're left with #{numbers.push(6)}"
